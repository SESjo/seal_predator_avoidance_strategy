...
data_dive %>% 
  filter(
    id == 2004021 &
      DiveTypeName == "Drift" & 
      lead(DiveTypeName, 1) == "Transit" &
      lead(DiveTypeName, 2) ==  "Forage") 
```
...
windrose_departure <- data_windrose %>%
  ...
  windrose(., grid = c(10, 20, 30,35), facet = T) +
...
# combine all
(windrose_departure + windrose_arrival) / label_x +
  plot_layout(
    heights = c(2,0),
    guides = "collect"
  ) &
  plot_annotation(tag_levels = c("A")) &
  theme(plot.tag.position = c(-0.05, 0.8),
...
# reorder layers
to_plot$layers <- to_plot$layers[c(1,3,2)]
...
inside_bar_plot = data_descriptive_area %>%
  ...
  scale_fill_manual(values = c("#004D40", "#E66100", "#9A6BEC", "#E1E826")) +
  new_scale_fill() +
  geom_col(
    show.legend = F,
    mapping = aes(
...
predator_map$layers <- predator_map$layers[c(1,4,3,2)]
```
...
predator_map$layers[[4]]$aes_params$fill_new_new = "#cdaa6d"
predator_map$layers[[4]]$aes_params$colour = "black"

# predator_map$layers[[4]]$aes_params$fill_new_new = "grey60"
# predator_map$layers[[4]]$aes_params$colour = "black"
