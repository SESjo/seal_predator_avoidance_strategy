...
# remove id == `2016014` and id = `2018002` (no benthic dives)
data_dive <- data_dive %>% 
  filter(!id %in% c("2016014", "2018002"))
```
  ...
  summarise(
    ...
    n_benthic_dives_ind_shark_or_orca = if_else(length(id[DiveTypeName == "Benthic"]) == 0,
                                                ...
                                                length(DiveNumber[DiveTypeName == "Benthic" &
                                                                    shark_or_orca == 1 &
                                                                    !is.na(Lat)])),
    hour_day_departure = first(hour(date_tz) +
                                 minute(date_tz) / 60 +
                                 second(date_tz) / (60 * 60)),
    hour_day_arrival = last(hour(date_tz) +
                              minute(date_tz) / 60 +
                              second(date_tz) / (60 * 60)),
    .groups = "drop"
  ) %>%
...
> More here: [https://stats.stackexchange.com/questions/265865/comparisons-of-circular-means](https://stats.stackexchange.com/questions/265865/comparisons-of-circular-means)
...
windrose_departure_pm <- data_windrose %>%
  group_by(id) %>%
  filter(date_tz == min(date_tz) & trip == "Post Molting") %>%
...
windrose_departure_pb <- data_windrose %>%
  group_by(id) %>%
  filter(date_tz == min(date_tz) & trip == "Post Breeding") %>%
  mutate(time = as.numeric(format(date_tz, format = "%H"))) %>%
  group_by(time, trip) %>%
  summarise(nb_ind_hour = n(), .groups = "drop") %>%
  mutate(origin = "Departure") %>%
...
> Good ressource to interpret Rayleigh test: [https://stats.stackexchange.com/questions/433396/interpretation-of-rayleigh-test-results](https://stats.stackexchange.com/questions/433396/interpretation-of-rayleigh-test-results)
...
broom::tidy(prop.test(x = c(prop_at_sea_dt %>% 
...
> Based on: [http://www.sthda.com/english/wiki/comparing-proportions-in-r](http://www.sthda.com/english/wiki/comparing-proportions-in-r)
