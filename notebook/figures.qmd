...
Let's add a condition based on the difference between the maximum depth reached and the bathymetry to define a benthic dive, *i.e.* if this difference is above xxx m then is not considered as a benthic dives.

```{r}
data_dive = data_dive %>%
  # let's duplicate Divetype and DiveType
  mutate(DiveType2 = DiveType,
         DiveTypeName2 = DiveTypeName) %>%
  # change DiveType to include condition on difference
  # between max depth and bathymetry
  mutate(
...
data_dive %>%
  ...
  summarise(
    n_ind = length(id),
    n_dives = sum(n_dives_ind),
    n_benthic_dives = sum(n_benthic_dives_ind),
    n_benthic_dives_proportion = sum(n_benthic_dives)/sum(n_dives_ind),
    ...
    n_dives_mean = mean(n_dives_ind),
    n_dives_plus_minus = "±",
    n_dives_sd = sd(n_dives_ind),
    n_benthic_dives_mean = mean(n_benthic_dives_ind),
    n_benthic_dives_plus_minus = "±",
    n_benthic_dives_sd = sd(n_benthic_dives_ind),
    hour_day_departure_mean = mean(hour_day_departure),
    hour_day_departure_plus_minus = "±",
    hour_day_departure_sd = sd(hour_day_departure),
    hour_day_arrival_mean = mean(hour_day_arrival),
    hour_day_arrival_plus_minus = "±",
    hour_day_arrival_sd = sd(hour_day_arrival)
  ) %>% 
  gt()%>% 
  tab_spanner(
    label = "% benthic dives within",
  ...
  cols_label(
    n_ind = "# seals",
    n_dives = "# dives",
    n_benthic_dives = "# benthic dives",
    n_benthic_dives_proportion = "% benthic dives",
...
* color columns
* circular mean and sd instead of regular
* add individuals without location data
:::
...
data_windrose %>%
  ...
  geom_boxplot() +
  labs(x = "Time of the day", y = "Average number of dives per individual")
```
...
data_windrose %>%
  ...
  ggplot(aes(x = day_night, y = nb_average_benthic_dive)) +
  geom_boxplot() +
  labs(x = "Time of the day", y = "Average number of dives per individual per hour")
```
