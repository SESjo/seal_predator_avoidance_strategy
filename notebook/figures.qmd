...
```{r}
# import the data
# data_dive <- readRDS("../export/data_dive_rev_3_inter.rds")
data_dive <- readRDS("../export/data_dive.rds")

# filter on seals departing from ANNU
data_dive <- data_dive %>%
  filter(DepartLoc == "ANNU")

# remove id == `2015006` (no benthic dives)
data_dive <- data_dive %>%
  filter(!id %in% c("2015006"))
```
...
#   mutate(
#     # change divetype
#     DiveType = if_else(DiveType == 3 & 
#                          !is.na(Lat) &
#                          (abs(bathy) - Maxdepth) > 100, 0, DiveType),
#     # create DiveTypeName
#     DiveTypeName = case_when(
#       DiveType == 0 ~ "Transit",
#       DiveType == 1 ~ "Forage",
#       DiveType == 2 ~ "Drift",
#       DiveType == 3 ~ "Benthic"
...
#     trip = if_else(trip == "PB", "Post Breeding", "Post Molting"),
#     # add date
#     date = as.Date(DateTime),
#     # add inside/outside each predators area
...
compute_table_1_a <- function(x){
  # if the sum of the column is below 0.1
    if(sum(x)<(0.1*2)){
    # it's the proportion of dives that are benthic
    # so we weighted average these by taking into account all dives
    y = weighted.mean(x, c(data_dive %>% filter(trip == "Post Breeding") %>% nrow(),
                           data_dive %>% filter(trip == "Post Molting") %>% nrow()))
    y = paste0(round(y*100,1),"%")
    return(y)
  # otherwise if below 1
  } else if(sum(x)<(1*2)){
    # it's the proportion of benthic dives within predator areas
    # so we weighted average these by taking into account only dives with location data
    y = weighted.mean(x, c(data_dive %>% filter(trip == "Post Breeding" & !is.na(Lat)) %>% nrow(),
                           data_dive %>% filter(trip == "Post Molting" & !is.na(Lat)) %>% nrow()))
    y = paste0(round(y*100,1),"%")
    return(y)
...
compute_table_1_b <- function(x) {
  ...
  if (all(is.numeric(x))) {
    ...
    if (sum(x) < 100) {
      ...
      y = psych::circadian.mean(c(rep(x[1], data_dive %>% 
                                        filter(trip == "Post Breeding" & !is.na(Lat)) %>% 
                                        pull(id) %>% 
                                        n_distinct()),
                                  rep(x[2], data_dive %>% 
                                        filter(trip == "Post Molting" & !is.na(Lat)) %>% 
                                        pull(id) %>% 
                                        n_distinct())),
                                na.rm = T)
      y = round(y, 1)
      ...
      y = weighted.mean(x, c(data_dive %>% 
                                        filter(trip == "Post Breeding") %>% 
                                        pull(id) %>% 
                                        n_distinct(), 
                             data_dive %>% 
                                        filter(trip == "Post Molting") %>% 
                                        pull(id) %>% 
                                        n_distinct()))
      y = round(y, 1)
    }
...
table_1_b <- data_dive %>%
  group_by(trip, id) %>%
   group_by(trip, id) %>%
  # create a column to say when start and end each trip by id
  mutate(is_arrival = if_else(DiveNumber == max(DiveNumber), T, F),
         is_departure = if_else(DiveNumber == min(DiveNumber), T, F)) %>% 
  # then summarise
  summarise(
    n_dives_ind_all = length(DiveNumber),
    n_dives_ind_w_loc = length(DiveNumber[!is.na(Lat)]),
    ...
    n_benthic_dives_ind_shark_or_orca = if_else(length(id[DiveTypeName == "Benthic"]) == 0,
                                                ...
                                                length(DiveNumber[DiveTypeName == "Benthic" &
                                                                    shark_or_orca == 1 &
                                                                    !is.na(Lat)])),
    hour_day_departure = hour(date_tz[is_departure == T]) +
                                 minute(date_tz[is_departure == T]) / 60 +
                                 second(date_tz[is_departure == T]) / (60 * 60),
    hour_day_arrival = hour(date_tz[is_arrival == T]) +
                              minute(date_tz[is_arrival == T]) / 60 +
                              second(date_tz[is_arrival == T]) / (60 * 60),
    .groups = "drop"
  ) %>%
  group_by(trip) %>%
  summarise(
...
hour_data <- data_dive %>%
  ...
  filter(!is.na(hour_day_departure) & !is.na(hour_day_arrival))

# compare the circular mean hour of departure between two types of trip
watson.williams.test(
  list(
    pb = hour_data %>% 
         filter(trip == "Post Breeding") %>% 
         pull(hour_day_departure) %>% 
         circular(., units = "hours"),
    pm = hour_data %>% 
         filter(trip == "Post Molting") %>% 
         pull(hour_day_departure) %>% 
         circular(., units = "hours"))
...
> More here: <https://stats.stackexchange.com/questions/265865/comparisons-of-circular-means>
...
departure_dp <- data_plot %>%
  ...
  scale_y_reverse(limits = rev(range(data_plot$depth))) +
...
```{r fig-1}
#| fig-cap: Snapshot of departure and arrival dive profiles for six seals. Departure and arrival were constrained to 36 hours. Shading indicates night and coloration indicates dive type.
#| fig-height: 7
...
data_windrose <- data_dive %>%
  # first we need to identify the start and the end of each trip by id
  group_by(id, trip) %>%
  mutate(is_arrival = if_else(DiveNumber == max(DiveNumber), T, F),
         is_departure = if_else(DiveNumber == min(DiveNumber), T, F)) %>% 
  # keep only the data of the first, last dives
  filter(is_arrival == T | is_departure == T) %>% 
  # ungroup 
  ungroup() %>% 
  # then, since we're looking at local time, we keep data with location
  filter(!is.na(Lat)) %>% 
  # change colnames to make getSunlightTimes works
  mutate(date = as.Date(date_tz),
         lat = Lat,
         lon = Lon) %>%
  ...
  mutate(getSunlightTimes(
    ...
    tz = tz_lookup_coords(lat = Lat,
                          lon = Lon,
                          method = "accurate")
  )) %>%
  # extract hours of sunset an sunrise
  mutate(h_sunrise = round(hour(sunrise) +
               minute(sunrise)/60 +
               second(sunrise)/(60*60)),
         h_sunset = round(hour(sunset) +
               minute(sunset)/60 +
               second(sunset)/(60*60)))
```
...
windrose_departure_pb <- data_windrose %>%
  filter(is_departure == T & trip == "Post Breeding") %>%
  mutate(time = as.numeric(format(date_tz, format = "%H"))) %>%
  group_by(time, trip) %>%
  summarise(nb_ind_hour = n_distinct(id), .groups = "drop") %>%
  mutate(origin = "Departure") %>%
  windrose(., 
           grid = c(5, 10), 
           facet_wrap = "origin",
           hour_sunrise = data_windrose %>%
             filter(is_departure == T &
                      trip == "Post Breeding") %>% 
             pull(h_sunrise) %>% 
...
windrose_departure_pm <- data_windrose %>%
  filter(is_departure == T & trip == "Post Molting") %>%
  mutate(time = as.numeric(format(date_tz, format = "%H"))) %>%
  group_by(time, trip) %>%
  summarise(nb_ind_hour = n_distinct(id), .groups = "drop") %>%
  mutate(origin = "Departure") %>%
...
windrose_arrival_pb <- data_windrose %>%
  filter(is_arrival == T & trip == "Post Breeding") %>%
  mutate(time = as.numeric(format(date_tz, format = "%H"))) %>%
  group_by(time, trip) %>%
  summarise(nb_ind_hour = n_distinct(id), .groups = "drop") %>%
  mutate(origin = "Arrival") %>%
  windrose(., grid = c(5, 10),
           legend_position = "top",
           facet_grid = T,
           hour_sunrise = data_windrose %>%
             group_by(id) %>% 
...
windrose_arrival_pm <- data_windrose %>%
  filter(is_arrival == T & trip == "Post Molting") %>%
  mutate(time = as.numeric(format(date_tz, format = "%H"))) %>%
  group_by(time, trip) %>%
  summarise(nb_ind_hour = n_distinct(id), .groups = "drop") %>%
  mutate(origin = "Arrival") %>%
...
```{r}
# departure PM
pm_dep_mean = psych::circadian.mean(
  data_windrose %>%
    filter(is_departure == T & 
             trip == "Post Molting") %>%
...
# departure PB
pb_dep_mean = psych::circadian.mean(
  data_windrose %>%
    filter(is_departure == T & 
             trip == "Post Breeding") %>%
    mutate(time = hour(date_tz) +
...
# arrival PM
pm_arr_mean = psych::circadian.mean(
  data_windrose %>%
    filter(is_arrival == T
           & trip == "Post Molting") %>%
    mutate(time = hour(date_tz) +
...
# arrival PB
pb_arr_mean = psych::circadian.mean(
  data_windrose %>%
    filter(is_arrival == T
           & trip == "Post Breeding") %>%
    mutate(time = hour(date_tz) +
...
```{r}
# functio to convert time in radian (probably already exist in some package...)
...
# arrival
test_arrival_pm = CircStats::r.test(
  data_windrose %>%
      filter(is_arrival == T
             & trip == "Post Molting") %>%
      mutate(time = hour(date_tz) +
    ...
    pull(time) %>%
    hour_time_2_radian(.))
test_arrival_pb = CircStats::r.test(
    data_windrose %>%
      filter(is_arrival == T
             & trip == "Post Breeding") %>%
      mutate(time = hour(date_tz) +
...
```{r tbl-no-include}
#| tbl-cap: (Not meant to be in the paper) Average hour of departure and arrival for post breeding and post molting trip with the result of the associated Rayleigh test.
...
data.frame(
  Departure = c(
    paste0(
      round(pb_dep_mean, 1),
      " (R = ",
      round(test_departure_pb$r.bar, 2),
      ", p-value ",
      ifelse(
        round(test_departure_pb$p.value, 2) == 0,
        "< 0.001",
        paste("= ", round(test_departure_pb$p.value, 2))
      ),
      ")"
    ),
    paste0(
      round(pm_dep_mean, 1),
      " (R = ",
      round(test_departure_pm$r.bar, 2),
      ", p-value ",
      ifelse(
        round(test_departure_pm$p.value, 2) == 0,
        "< 0.001",
        paste("= ", round(test_departure_pm$p.value, 2))
      ),
      ")"
    )
  ),
    ...
    paste0(
      round(pb_arr_mean, 1),
      " (R = ",
      round(test_arrival_pb$r.bar, 2),
      ", p-value ",
      ifelse(
        round(test_arrival_pb$p.value, 2) == 0,
        "< 0.001",
        paste("= ", round(test_arrival_pb$p.value, 2))
      ),
      ")"
    ),
    paste0(
      round(pm_arr_mean, 1),
      " (R = ",
      round(test_arrival_pm$r.bar, 2),
      ", p-value ",
      ifelse(
        round(test_arrival_pm$p.value, 2) == 0,
        "< 0.001",
        paste("= ", round(test_arrival_pm$p.value, 2))
      ),
      ")"
    )
  ),
  rownames = c("Post Breeding", "Post Molting")) %>%
  gt(rowname_col = "rownames") %>% 
  cols_label(
    Arrival = "Hour of Arrival",
    Departure = "Hour of Departure"
  )
```

> Good ressource to interpret Rayleigh test: <https://stats.stackexchange.com/questions/433396/interpretation-of-rayleigh-test-results>
...
df_kernel_dens_benthic <- data_dive %>%
  ...
  select(lat = Lat, long = Lon, id, DiveTypeName) %>%
...
# df_kernel_dens_sf_kde_benthic <- st_kde(df_kernel_dens_sf_benthic,
#                                         H = ks::Hpi(sf::st_coordinates(df_kernel_dens_sf_benthic),
#                                                          pilot = "dunconstr"))

# df_kernel_dens_sf_kde_benthic <- st_kde(df_kernel_dens_sf_benthic,
#                                         H = ks::Hlscv.diag(sf::st_coordinates(df_kernel_dens_sf_benthic)))
...
p1 <- trip_zoom_in +
  ...
  # scale_fill_grey() +
  # legend
  labs(fill = "Probability contours") +
  # legend at the top
  theme(legend.position = "top") +
...
```{r fig-map-3}
#| fig-cap: "Spatial overlap of benthic dives performed by northern elephant seals (n = 401), and the shark (yellow polygon) and orca (red polygon) distribution area along the west coast (adapted from Jorgenson, et al. 2019) with a kernel density estimation representing the occurrence of all the benthic dives."
#| fig-width: 8

# load image icon
shark_img <- readPNG("../img/shark_custom.png", native = T)
orca_img <- readPNG("../img/orca_custom.png", native = T)
...
```{r fig-map-3-no-run}
#| fig-cap: "Same as the previous one, but with the benthic dives (black dots). We can see that some benthic happen outside orca areas..."
#| fig-width: 8

# draw the map
predator_map +
  ggspatial::geom_spatial_point(data = data_dive %>% 
               filter(!is.na(Lat) &
                        DiveTypeName == "Benthic"),
             aes(x = Lon, y = Lat), alpha = 0.2, size = 1) +
...
```{r}
# https://mycolor.space/?hex=%23008C49&sub=1
# build palette color
cols_ind_1 <- c("#008c49", "#00c9d3", "#bffcfd", "#458081")
# test for color blind
# => https://davidmathlogic.com/colorblind/#%23008C49-%2300C9D3-%23BFFCFD-%23458081
cols_ind_2 <- c("#008c49", "#002e00", "#00c9d3", "#deb887")
# test for color blind 
# => https://davidmathlogic.com/colorblind/#%23008C49-%23002E00-%2300C9D3-%23DEB887
```
...
ylab <- dist_coast_prop$labels$y
bathy_prop$labels$y <- dist_dep_prop$labels$y <-" "
```
...
(guide_area() + bathy_prop + dist_coast_prop + dist_dep_prop) +
  theme(plot.margin = unit(c(0,.02,-1,.05),units = "npc")) +
  plot_annotation(tag_levels = 'A') +
  plot_layout(nrow = 5, guides = "collect")
# grid::grid.draw(grid::textGrob(ylab, x = 0.07, y = 0.55, rot = 90))
```
...
prop_at_sea <- prop_at_sea_dt %>% 
  ...
  annotate("text", x = 15, y = 0.43, label = "Arrival") +
  # geom_curve(
  #   data =   tibble(
  #     x1 = c(4, 17),
  #     x2 = c(1.5, 19.5),
...
```{r}
# test to compare the proportion between arrival and departure
z_test <- prop.test(x = c(prop_at_sea_dt %>% 
                  filter(day_class == first(day_class)) %>% 
                  pull(nb_benthic_class), 
                prop_at_sea_dt %>% 
                  filter(day_class == last(day_class)) %>% 
                  pull(nb_benthic_class)), 
          n = c(prop_at_sea_dt %>% 
                  filter(day_class == first(day_class)) %>% 
                  pull(nb_total_class), 
...
sqrt(z_test$statistic)
```

> $H_{0}$ is rejected, so we can say that the porportion of benthic dives at arrival is significantly superior than departure!

> Based on: <http://www.sthda.com/english/wiki/comparing-proportions-in-r>

> and <https://bookdown.org/danieljcarter/r4steph/z-test-for-proportions.html>
  ...
  summarise(
    ...
    n_benthic_dives_ind_shark_or_orca = if_else(length(id[DiveTypeName == "Benthic"]) == 0,
                                                ...
                                                length(DiveNumber[DiveTypeName == "Benthic" &
                                                            shark_or_orca == 1 &
                                                            !is.na(Lat)])),
    hour_day_departure = hour(date_tz[DiveNumber == min(DiveNumber)]) +
                                 minute(date_tz[DiveNumber == min(DiveNumber)]) / 60 +
                                 second(date_tz[DiveNumber == min(DiveNumber)]) / (60 * 60),
    hour_day_arrival = hour(date_tz[DiveNumber == max(DiveNumber)]) +
                                 minute(date_tz[DiveNumber == max(DiveNumber)]) / 60 +
                                 second(date_tz[DiveNumber == max(DiveNumber)]) / (60 * 60),
    .groups = "drop"
  ) %>%
  group_by(year) %>%
  summarise(
...
```{r fig-map-1}
#| fig-cap: "Map of kernel density estimation representing the spatial distribution of Benthic, Drift, Forage, and Transit dives performed by northern elephant seals throughout their trip to sea. Probability contours represent different levels of probability density."
#| fig-height: 5
#| fig-asp: 0.6
...
prop_at_sea_trip <- data_dive %>%
  ...
  # group by day/dive_type/seal
  group_by(nb_days_departure, DiveTypeName, id, trip) %>%
  # count number of dives
  summarise(nb_daily_dives_type = n(), .groups = "drop") %>%
  # group by day/seal
  group_by(nb_days_departure, id) %>%
  ...
  mutate(nb_daily_dives = sum(nb_daily_dives_type)) %>%
  # order by seals/date
  arrange(id, nb_days_departure, trip) %>%
  # perform our calculation per seal
  group_by(id, trip) %>%
  # calculate of the Proportion of time since departure
  mutate(perc_time_at_sea = round(
...
```{r fig-prop-suppmat-2}
#| fig-cap: "Proportion of benthic dives performed during trip at sea expressed as a percentage of time spent at sea during post breeding and post molting trip. Time spent at sea is classified into intervals of five percent for graphical representation."
#| fig-height: 3
# display
prop_at_sea_trip
```
...
#| include: false
#| eval: false
fwrite(data_dive[,.(id, date_start = date, date_end = date + Dduration, lat = Lat, lon = Lon, dive_number = DiveNumber, benthic_index = BenthicDiveIndex, corner_index = CornerIndex, dive_type_name = DiveTypeName, dive_type = DiveType)], "./export/dive_data.csv")
```
