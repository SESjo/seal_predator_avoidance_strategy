...
Based on here: [https://stackoverflow.com/questions/59566553/calculate-night-length-in-r](https://stackoverflow.com/questions/59566553/calculate-night-length-in-r)
...
```{r fig-boxplot-all}
#| fig-cap: "Boxplot of the number of (all) benthic dives per hour and individual accross day-time and night-time"
#| 
data_plot = data_dive %>%
  # get rid of data without location information
  filter(!is.na(Lat)) %>%
...
```{r fig-fit-data-all}
#| fig-cap: Result from `descdist` function to determine the best distribution to use for data modelling

# determine what distribution would best describe our data
descdist(data_plot$nb_average_benthic_dive, discrete = FALSE)
```

Based on @fig-fit-data-all, it seems that a `gamma` distribution would be the best to model this dataset.

```{r}
# model
mdl_all = glmer(nb_average_benthic_dive ~ day_night +
...
For more information: [https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#overdispersion](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#overdispersion)

```{r}
# model
mdl_all_extra = glmer(nb_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
...
```{r fig-boxplot-50}
#| fig-cap: "Boxplot of the number of (diff<50) benthic dives per hour and individual accross day-time and night-time"
#| 
data_plot <- data_dive %>%
  # get rid of data without location information
  filter(!is.na(Lat)) %>%
...
Based on @fig-fit-data-50, it seems that a `gamma` distribution would be the best to model this dataset.

```{r}
# model
mdl_50 = glmer(nb_average_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                 (1|id), 
                family = Gamma,
                data = data_plot %>% mutate(day_night = factor(day_night,
...
```{r}
# model
mdl_50_extra = glmer(nb_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                  scale(mean_period_duration) +
                 (1|id), 
                family = "poisson",
...
```{r fig-boxplot-100}
#| fig-cap: "Boxplot of the number of (diff<100) benthic dives per hour and individual accross day-time and night-time"
#| 
data_plot <- data_dive %>%
  # get rid of data without location information
  filter(!is.na(Lat)) %>%
...
Based on @fig-fit-data-100, it seems that a `gamma` distribution would be the best to model this dataset.

```{r}
# model
mdl_100 = glmer(nb_average_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                 (1|id), 
                family = Gamma,
                data = data_plot %>% mutate(day_night = factor(day_night,
...
```{r}
# model
mdl_100_extra = glmer(nb_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                  scale(mean_period_duration) +
                 (1|id), 
                family = "poisson",
...
```{r fig-boxplot-150}
#| fig-cap: "Boxplot of the number of (diff<150) benthic dives per hour and individual accross day-time and night-time"
#| 
data_plot <- data_dive %>%
  # get rid of data without location information
  filter(!is.na(Lat)) %>%
...
Based on @fig-fit-data-150, it seems that a `gamma` distribution would be the best to model this dataset.

```{r}
# model
mdl_150 = glmer(nb_average_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                 (1|id), 
                family = Gamma,
                data = data_plot %>% mutate(day_night = factor(day_night,
...
```{r}
# model
mdl_150_extra = glmer(nb_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                  scale(mean_period_duration) +
                 (1|id), 
                family = "poisson",
...
```{r fig-boxplot-200}
#| fig-cap: "Boxplot of the number of (diff<200) benthic dives per hour and individual accross day-time and night-time"
#| 
data_plot <- data_dive %>%
  # get rid of data without location information
  filter(!is.na(Lat)) %>%
...
Based on @fig-fit-data-200, it seems that a `gamma` distribution would be the best to model this dataset.

```{r}
# model
mdl_200 = glmer(nb_average_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                 (1|id), 
                family = Gamma,
                data = data_plot %>% mutate(day_night = factor(day_night,
...
```{r}
# model
mdl_200_extra = glmer(nb_benthic_dive ~ day_night +
                 scale(mean_dive_duration) +
                  scale(mean_period_duration) +
                 (1|id), 
                family = "poisson",
...
# Boxplot of the number of benthic dive (day vs. night)

:::{.panel-tabset}

## All

```{r fig-boxplotbis-all}
...
data_inter = data_dive %>%
  filter(!is.na(Lat) &
           Dduration > 4000) %>%
  arrange(id, date) %>% 
  mutate(date_post = date + Dduration,
         diff_time = as.numeric(c(difftime(tail(date, -1), 
                                           head(date_post, -1)),0), 
                                units = "mins"))

colPal <- scales::col_numeric(palette = "RdYlGn",
...
trip +
  ...
  guides() +
  # title
  labs(title = "Dives longer than 4000 seconds",
